...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::Begin("Window");
		ImGui::Text("R : Reset");
		ImGui::SliderFloat3("sphere.center", &sphere.center_.x, -10.0f, 10.0f);
		...
		if (Collision::IsCollision(sphere, plane)) {
			Sphere::DrawSphere(sphere, viewProjectionMatrix, viewportMatrix, 0xFF0000FF);
		}
		else {
			Sphere::DrawSphere(sphere, viewProjectionMatrix, viewportMatrix, 0xFFFFFFFF);
		}
